...
class C3(nn.Module):  #n的值代表内部残差结构所循环的次数,即为base_depth
    # CSP Bottleneck with 3 convolutions
...
class GhostConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):  # ch_in, ch_out, kernel, stride, groups
        ...
        self.cv2 = Conv(c_, c_, 5, 1, None, c_, act)    #廉价操作
...
class h_sigmoid(nn.Module):                           #改进的h-swish函数： h-swish[X]=X*ReLU6(X+3)/6
    def __init__(self, inplace=True):                 #ReLU6是ReLU函数，最大值为6，最小值为0
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, hidden_dim, kernel_size, stride, use_se, use_hs):
        ...
        if inp == hidden_dim:    #通道数相同 则为DW卷积-PW卷积
            self.conv = nn.Sequential(
                # dw
                nn.Conv2d(hidden_dim, hidden_dim, kernel_size, stride, (kernel_size - 1) // 2, groups=hidden_dim,
                          bias=False),
                nn.BatchNorm2d(hidden_dim),   #BN层
                h_swish() if use_hs else nn.ReLU(inplace=True),   #激活层
                # Squeeze-and-Excite
...
class eca_block(nn.Module):
    def __init__(self, channel, b=1, gamma=2):
        ...
        kernel_size = kernel_size if kernel_size % 2 else kernel_size + 1    #使得卷积核为奇数
...
class E_ELAN(nn.Module):
    def __init__(self,c1,c2,flg=False):
        ...
        '''
            修改：C2变为了输出通道数，中间层输出通道数由c_代替。
                 c_有修改空间，且conv2的k值有修改空间，极大影响了整个模型的轻量化程度
                 ！或许可以在在某些地方使用不同的E_ELAN
        '''
