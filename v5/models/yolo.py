...
'''
    使用YOLOXHead 需要替换Detect中的self.m，同时注释掉Model中的__init__的self._initialize_biases()
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.register_buffer('anchor_grid', a.clone().view(self.nl, 1, -1, 1, 1, 2))  # shape(nl,1,na,1,1,2)
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
        #self.m = nn.ModuleList(DecoupledHead(x, nc, 1, anchors) for x in ch)  # output conv
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        C3TR, C3SPP, C3Ghost, CoorAttention,PSAModule,CAConv,CABottleneck,CAC3,SEConv,SEBottleneck,SEC3,
        conv_bn_relu_maxpool,Shuffle_Block,E_ELAN,eca_block,SPPCSPC, GhostSPPCSPC,MPConv, conv_bn_relu_maxpool, Shuffle_Block,h_swish,Mobile_SELayer,conv_bn_hswish,MobileNet_Block,
        RepConv,DownC,CBAM,Conv3BN,InvertedResidual,stem, FusedMBConv, MBConv, CSPStage]:
            #c1:当前层的输入的channeL数c2:当前层的输出的channeL数(初定)
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not output no=75 只有最后一层c2=no 最后一层不用控制宽度，输出channeL必须是no
                # width gain控制宽度  如V5s: c2*0.5  c2:当前层的最终输出的channeL数(间接控制宽度)
                c2 = make_divisible(c2 * gw, 8)
 
            #在初始arg的基础上更新加入当前层的输入channel并更新当前层
            # [ in_ channel, out_ channel, *args[1:]]
            args = [c1, c2, *args[1:]]
            #如果当前层是BottLeneckCSP/C3/C3TR/C3Ghost, 则需要在args中加入bottLeneck的个数
            # [in_ channel, out_ channel, Bottleneck的个数n，bool(True表示有shortcut 默认，反之无) ]
            if m in [BottleneckCSP, C3, C3TR, C3Ghost, SPPCSPC, GhostSPPCSPC,DownC,CSPStage]:
